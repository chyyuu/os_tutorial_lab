...
umain(int argc, char **argv) {
  ...
  do { if (!(shcwd != ((void *)0))) { __panic("ucore/user/sh.c", 197, "assertion failed: %s", "shcwd != NULL"); } } while (0);

  while ((buffer = readline((interactive) ? "$ " : ((void *)0))) != ((void *)0)) {
    cprintf("\ndo %s\n", buffer);
    shcwd[0] = '\0';
    if ((pid = fork()) == 0) {
      ret = runcmd(buffer);
    ...
    do { if (!(pid >= 0)) { __panic("ucore/user/sh.c", 206, "assertion failed: %s", "pid >= 0"); } } while (0);
