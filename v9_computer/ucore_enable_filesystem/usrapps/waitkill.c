...
umain(void) {
  ...
  do { if (!(pid1 > 0)) { __panic("ucore/user/waitkill.c", 45, "assertion failed: %s", "pid1 > 0"); } } while (0);
  ...
  if (pid2 > 0) {
    ...
    waitpid(pid1, ((void *)0));
    __panic("ucore/user/waitkill.c", 53, "waitpid %d returns\n", pid1);
  }
  else {
    kill(pid1);
  }
  __panic("ucore/user/waitkill.c", 58, "FAIL: T.T\n");
}
