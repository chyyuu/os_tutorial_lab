...
umain(void) {
  ...
  do { if (!(pid > 0)) { __panic("ucore/user/exit.c", 30, "assertion failed: %s", "pid > 0"); } } while (0);
  ret = waitpid(pid, &code);
  do { if (!(ret== 0 && code == magic)) { __panic("ucore/user/exit.c", 32, "assertion failed: %s", "ret== 0 && code == magic"); } } while (0);
  do { if (!(waitpid(pid, &code) != 0 && wait() != 0)) { __panic("ucore/user/exit.c", 33, "assertion failed: %s", "waitpid(pid, &code) != 0 && wait() != 0"); } } while (0);
