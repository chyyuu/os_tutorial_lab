...
umain(void) {
  ...
  do { if (!(pid > 0)) { __panic("ucore/user/badarg.c", 15, "assertion failed: %s", "pid > 0"); } } while (0);
  do { if (!(waitpid(-1, ((void *)0)) != 0)) { __panic("ucore/user/badarg.c", 16, "assertion failed: %s", "waitpid(-1, NULL) != 0"); } } while (0);
  do { if (!(waitpid(pid, (void *)0xC0000000) != 0)) { __panic("ucore/user/badarg.c", 17, "assertion failed: %s", "waitpid(pid, (void *)0xC0000000) != 0"); } } while (0);
  do { if (!(waitpid(pid, &exit_code) == 0 && exit_code == 0xbeaf)) { __panic("ucore/user/badarg.c", 18, "assertion failed: %s", "waitpid(pid, &exit_code) == 0 && exit_code == 0xbeaf"); } } while (0);
  cprintf("badarg pass.\n");
  return 0;
}
