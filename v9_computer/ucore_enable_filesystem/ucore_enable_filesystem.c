...
int consoleread(struct inode *ip, char *dst, int n)
{
  ...
  while (n > 0) {
    if (input.r == input.w && n < target) break;
...
iderw(struct buf *b)
{
  ...
  if (b->sector >= (1024 * 4096 / 4096)) panic("iderw: sector out of range");

  if (b->flags & B_DIRTY) {
    b->flags &= ~B_DIRTY;
    memcpy(memdisk + b->sector*4096, b->data, 4096);
  } else
    memcpy(b->data, memdisk + b->sector*4096, 4096);
  b->flags |= B_VALID;
}
...
int readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for (tot = n; tot; tot -= m, off += m, dst += m) {
    bp = bread(bmap(ip, off/4096));
    if ((m = 4096 - off%4096) > tot) m = tot;
    memcpy(dst, bp->data + off%4096, m);
    brelse(bp);
  }
  return n;
}
...
{
  uint n = DIRSIZ;
  while (n) { if (!*p || *p != *q) return *p - *q; n--; p++; q++; }
  return 0;
}
...
struct inode *dirlookup(struct inode *dp, char *name, uint *poff)
{
  ...
  for (off = 0; off < dp->size; off += sizeof(de)) {
    ...
    if (de.d_ino && !namecmp(name, de.d_name)) {
      if (poff) *poff = off;
...
int writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  if (off > ip->size || off + n < off) return -1;
  if (off + n > (NDIR + NIDIR*1024)*4096) return -1;

  for (tot = n; tot; tot -= m, off += m, src += m) {
    bp = bread(bmap(ip, off/4096));
    if ((m = 4096 - off%4096) > tot) m = tot;
    memcpy(bp->data + off%4096, src, m);
    bwrite(bp);
    brelse(bp);
  }
  if (n > 0 && off > ip->size) {
...
struct file *getf(uint fd) {
  return (fd < 16) ? current->ofile[fd] : 0;
}
...
fileclose(struct file *f)
{
  ...
  memcpy(&ff, f, sizeof(struct file));
  f->ref = 0;
  f->type = FD_NONE;
  splx(e);
  ...
  case FD_SOCKET:
  case FD_RFS: panic("file close no socket. ");
  }
}
...
load_icode(struct inode *ip, int argc, char **argv) {
  ...
  if ((ret = mm_map(mm, 0x00200000, ip->size + hdr.bss, vm_flags, ((void *)0))) != 0) {
  ...
  end = 0x00200000 + ip->size - sizeof(struct elfhdr);
  ...
  end = 0x00200000 + ip->size - sizeof(struct elfhdr) + hdr.bss;
  if (start < la) {
