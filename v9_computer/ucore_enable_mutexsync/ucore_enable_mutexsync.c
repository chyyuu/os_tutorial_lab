...
int do_yield(void);
int do_execve(char *name, size_t len, unsigned char *binary, size_t size);
...
wait_queue_add(wait_queue_t *queue, wait_t *wait) {
  assert(list_empty(&(wait->wait_link)) && wait->proc != ((void *)0));
...
wait_queue_next(wait_queue_t *queue, wait_t *wait) {
  ...
  if (le != &(queue->wait_head)) {
    return ((wait_t *)((char *)((le)) - ((size_t)(&((wait_t *)0)->wait_link))));
  }
  return ((void *)0);
}
...
wakeup_first(wait_queue_t *queue, uint32_t wakeup_flags, bool del) {
  wait_t *wait;
  if ((wait = wait_queue_first(queue)) != ((void *)0)) {
...
wakeup_queue(wait_queue_t *queue, uint32_t wakeup_flags, bool del) {
 ...
 wakeup_wait(queue, wait, wakeup_flags, 1);
      } while ((wait = wait_queue_first(queue)) != ((void *)0));
    }
    else {
      do {
 wakeup_wait(queue, wait, wakeup_flags, 0);
      } while ((wait = wait_queue_next(queue, wait)) != ((void *)0));
    }
  }
}
...
void page_init(void) {
  ...
  for (i = 0; i < 4096 * 512; i++)
    memdisk[i] = *(char*)(maxpa - 1024 * 4096 + i);
...
add_timer(timer_t *timer) {
  ...
  do { intr_flag = __intr_save(); } while (0);
  {
    assert(timer->expires > 0 && timer->proc != ((void *)0));
    assert(list_empty(&(timer->timer_link)));
    le = list_next(&timer_list);
    while (le != &timer_list) {
      next = ((timer_t *)((char *)((le)) - ((size_t)(&((timer_t *)0)->timer_link))));
...
run_timer_list(void) {
  ...
  do { intr_flag = __intr_save(); } while (0);
  {
    le = list_next(&timer_list);
    if (le != &timer_list) {
      timer = ((timer_t *)((char *)((le)) - ((size_t)(&((timer_t *)0)->timer_link))));
 ...
 if (proc->wait_state != 0) {
   assert(proc->wait_state & 0x80000000);
 }
 else {
...
static void __up(semaphore_t *sem, uint32_t wait_state) {
  ...
  do { intr_flag = __intr_save(); } while (0);
  {
    if ((wait = wait_queue_first(&(sem->wait_queue))) == ((void *)0)) {
      sem->value++;
    }
    else {
...
static uint32_t __down(semaphore_t *sem, uint32_t wait_state) {
  ...
  do { intr_flag = __intr_save(); } while (0);
  do { if (wait_in_queue(wait)) { wait_queue_del(&(sem->wait_queue), wait); } } while (0);
  __intr_restore(intr_flag);;
...
down(semaphore_t *sem) {
  uint32_t flags = __down(sem, 0x00000100);
  assert(flags == 0);
}
...
struct proc_struct *philosopher_proc_sema[5];

void phi_test_sema(i)
{
  if(state_sema[i]==1&&state_sema[(i-1+5)%5]!=2
     &&state_sema[(i+1)%5]!=2)
  {
    state_sema[i]=2;
    up(&s[i]);
...
int philosopher_using_semaphore(void * arg)
{
  int i, iter=0;
  i=(int)arg;
  printf("I am No.%d philosopher_sema\n",i);
  while(iter++<4)
  {
    printf("Iter %d, No.%d philosopher_sema is thinking\n",iter,i);
    do_sleep(10);
    phi_take_forks_sema(i);

    printf("Iter %d, No.%d philosopher_sema is eating\n",iter,i);
    do_sleep(10);
    phi_put_forks_sema(i);

  }
  printf("No.%d philosopher_sema quit\n",i);
  return 0;
}

struct proc_struct *philosopher_proc_condvar[5];
int state_condvar[5];
monitor_t mt, *mtp;

void check_sync_init() {
  mtp = &mt;
}

void phi_test_condvar (i) {
  if(state_condvar[i]==1&&state_condvar[(i-1+5)%5]!=2
     &&state_condvar[(i+1)%5]!=2) {
    printf("phi_test_condvar: state_condvar[%d] will eating\n",i);
    state_condvar[i] = 2;
    printf("phi_test_condvar: signal self_cv[%d] \n",i);
...
void phi_take_forks_condvar(int i) {
  ...
  state_condvar[i]=1;
  phi_test_condvar(i);
  while (state_condvar[i] != 2) {
    printf("phi_take_forks_condvar: %d didn't get fork and will wait\n",i);
    cond_wait(&mtp->cv[i]);
  }
...
int philosopher_using_condvar(void * arg) {
  ...
  int i, iter=0;
  i=(int)arg;
  printf("I am No.%d philosopher_condvar\n",i);
  while(iter++<4)
  {
    printf("Iter %d, No.%d philosopher_condvar is thinking\n",iter,i);
    do_sleep(10);
    phi_take_forks_condvar(i);

    printf("Iter %d, No.%d philosopher_condvar is eating\n",iter,i);
    do_sleep(10);
    phi_put_forks_condvar(i);

  }
  printf("No.%d philosopher_condvar quit\n",i);
  return 0;
}
...
void check_sync(void){
  ...
  for(i=0; i<5; i++) {
    sem_init(&s[i], 0);
    pid = kernel_thread(philosopher_using_semaphore, (void *)i, 0);
    if (pid <= 0) {
  ...
  for(i=0; i<5; i++) {
    state_condvar[i]=0;
    pid = kernel_thread(philosopher_using_condvar, (void *)i, 0);
    if (pid <= 0) {
...
set_proc_name(struct proc_struct *proc, char *name) {
  ...
  return memcpy(proc->name, name, 50);
}
...
get_proc_name(struct proc_struct *proc) {
  ...
  return memcpy(name, proc->name, 50);
}
...
do_sleep(unsigned int time) {
  ...
  current->state = PROC_SLEEPING;
  current->wait_state = (0x00000002 | 0x80000000);
  add_timer(timer);
  __intr_restore(intr_flag);;
